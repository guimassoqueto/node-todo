import { FieldComparisonValidator } from "@src/adapters/controllers/helpers/validation/field-comparison-validator";
import { RequiredFieldValidator } from "@src/adapters/controllers/helpers/validation/required-field-validator";
import { Validator } from "@src/adapters/controllers/helpers/validation/validator";
import { ValidatorComposite } from "@src/adapters/controllers/helpers/validation/validator-composite";
...
export function makeSignUpControllerFactory(): Controller {
  ...
  return new SignUpController(
    validator, loadUserByEmail, saveUserDb
  )
}

function makeSignupValidations(): ValidatorComposite {
  const validators: Validator[] = []
  for (const requiredField of ["name", "email", "password", "passwordConfirm"]) {
